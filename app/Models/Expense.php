...
class Expense extends Model
{
    ...
    public static function statsForUser(int $userId): array
    {
        ...
        $counts = static::where('user_id', $userId)
            ->selectRaw('COUNT(*) as total_items, SUM(paid) as paid_items')
            ->first();
    ...
    public static function categoryStatsForUser(int $userId): array
    {
        return static::where('expenses.user_id', $userId)
            ...
            ->selectRaw('
                ...
                COALESCE(SUM(CASE WHEN expenses.paid != 0 AND expenses.paid IS NOT NULL THEN expenses.quantity * expenses.unit_price * expenses.frequency ELSE 0 END), 0) as paid_total,
                COALESCE(SUM(CASE WHEN expenses.paid = 0 OR expenses.paid IS NULL THEN expenses.quantity * expenses.unit_price * expenses.frequency ELSE 0 END), 0) as unpaid_total,
                SUM(CASE WHEN expenses.paid != 0 AND expenses.paid IS NOT NULL THEN 1 ELSE 0 END) as paid_count,
                SUM(CASE WHEN expenses.paid = 0 OR expenses.paid IS NULL THEN 1 ELSE 0 END) as unpaid_count
            ')
